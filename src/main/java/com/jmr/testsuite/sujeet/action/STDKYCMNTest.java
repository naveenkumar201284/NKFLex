...
import com.jmr.testsuite.sujeet.page.STDKYCMN;

import io.github.millij.poi.ss.reader.XlsxReader;

@ContextConfiguration(classes = SpringConfiguration.class)
public class STDKYCMNTest extends AbstractTestNGSpringContextTests {
	...
	private STDKYCMN data;
	private final String screenId = "STDKYCMN";
	private final String screenName = "KYC Maintenance";

	private List<STDKYCMN> dataList;
	private List<STDKYCMN> resultList;

	@PostConstruct
	public void initSetup() throws Exception {
		fcubs.setScreenDetails(screenId, screenName);
		fcubs.launchApp();
		dataList = fcubs.loadDataFromExcel(STDKYCMN.class, screenId);
		...
		for (STDKYCMN data : dataList) {
			try {
				...
				if (data.getRunMode().equalsIgnoreCase("Yes") || data.getRunMode().equalsIgnoreCase("Y")) {
					...
					if (data.getNewOperation().equalsIgnoreCase("Yes")) {
						fcubs.clickNew();
						Thread.sleep(2000);
						...
						// if(data.getKycCustomerType() == "Retail Customer")

						if (data.getKycretail().equalsIgnoreCase("Yes")) {

							fcubs.openChildScreenByTitle("Retail Customer");
							/*
							 * fcubs.switchToFrameByTitle("KYC Retail Profile");
							 */ //
							 ...
							 * fcubs.populateTextById("BLK_BFNOWNCMP__CMPNAME",
							 * data.getCompanyNameRet());
							 * fcubs.populateTextById("BLK_BFNOWNCMP__LOCATION",
							 * data.getLocationRet());
							 * fcubs.populateTextById("BLK_BFNOWNCMP__HOLDNGI",
							 * data.getPercentHoldingRet());
							 * fcubs.populateTextById(
							 * "BLK_BFNOWNCMP__ANNLINCMI",
							 * data.getAnnualIncomeRet());
							 * fcubs.populateTextById("BLK_BFNOWNCMP__BUSSLINE",
							 * data.getLineOfBusinessRet());
							 */
							 ...
							 * fcubs.populateTextById("BLK_RELCMPACC__CMPACC",
							 * data.getCompanyAccNumberRet());
							 * 
							 * fcubs.clickButtonById("cmdAddRow_BLK_PRSNLACC");
							 * fcubs.populateTextById("BLK_PRSNLACC__PRSNLACC",
							 * data.getPersonalAccNumberRet());
							 */
						...
						} else if (data.getKycretail().equalsIgnoreCase("No")) {

							fcubs.openChildScreenByTitle("Corporate Customer");
							 ...
							 * 
							 * fcubs.populateTextById(
							 * "BLK_CRPSUPPLER__SUPPLRNAME",
							 * data.getSupplierNameCorp());
							 * fcubs.populateTextById(
							 * "BLK_CRPSUPPLER__COUNTRES",
							 * data.getSupplierCountryCorp());
							 * fcubs.populateTextById(
							 * "BLK_CRPSUPPLER__SETTLMNTMOD",
							 * data.getSupplierSettlementModeCorp());
							 */

							/*
							 * fcubs.clickButtonById("cmdAddRow_BLK_CRPCLIENT");
							 * fcubs.populateTextById(
							 * "BLK_CRPCLIENT__CLIENTNAME",
							 * data.getClientNameCorp());
							 * fcubs.populateTextById("BLK_CRPCLIENT__COUNTRES",
							 * data.getClientCountryCorp());
							 * fcubs.populateTextById("BLK_CRPCLIENT__PAYMODE",
							 * data.getClientPaymentModeCorp());
							 */
							 ...
							 * else { fcubs.clickButtonById("CVS_FIN");
							 * fcubs.switchToFrameByTitle(
							 * "Account Financial Details");
							 * 
							 * fcubs.populateTextById("BLK_KYCFINCL__BNKADDR",
							 * data.getBankAddressFin());
							 * fcubs.populateTextById("BLK_KYCFINCL__COUNTRY",
							 * data.getCountryFin());
							 * 
							 * fcubs.clickButtonById("cmdAddRow_BLK_TXNPRDCT");
							 * fcubs.populateTextById("BLK_TXNPRDCT__TXNPRDCT",
							 * data.getTransactionProductsFin());
							 * fcubs.populateTextById("BLK_TXNPRDCT__RATING",
							 * data.getRatingFin()); fcubs.populateTextById(
							 * "BLK_TXNPRDCT__NOTXNMNTHLYI",
							 * data.getMonthlyNumberOfTransactionsFin());
							 * fcubs.populateTextById(
							 * "BLK_TXNPRDCT__MNTHLYAMTI",
							 * data.getMonthlyAmountFin());
							 * 
							 * fcubs.clickButtonById("cmdAddRow_BLK_MGMNTDTL");
							 * fcubs.populateTextById("BLK_MGMNTDTL__NAME",
							 * data.getMgmntNameFin());
							 * fcubs.populateTextById("BLK_MGMNTDTL__DESIGNATN",
							 * data.getMgmntDesginationFin());
							 * 
							 * fcubs.clickButtonById("cmdAddRow_BLK_SHAREHOLDRS"
							 * );
							 * fcubs.populateTextById("BLK_SHAREHOLDRS__NAME",
							 * data.getShareholderNameFin());
							 * fcubs.populateTextById(
							 * "BLK_SHAREHOLDRS__HOLDINGI",
							 * data.getShareholderHoldingFin());
							 * fcubs.populateTextById(
							 * "BLK_SHAREHOLDRS__ADDRESS",
							 * data.getShareholderAddressFin());
							 * fcubs.populateTextById("BLK_SHAREHOLDRS__CITY",
							 * data.getShareholderCityFin());
							 * fcubs.populateTextById(
							 * "BLK_SHAREHOLDRS__COUNTRY",
							 * data.getShareholderCountryFin());
							 * 
							 * fcubs.clickBtnOk();
							 * 
							 * }
					...
					else if (data.getEnterQuery().equalsIgnoreCase("Yes")) {
						fcubs.clickEnterQuery();						
						fcubs.populateTextById("BLK_KYCMASTER__KYCREFNO", data.getKycReferenceNumber());						
